...
GradientOverlay = (function(){
	...
	var ContentLoaded=function(c,h){function k(){try{l.doScroll("left")}catch(a){setTimeout(k,50);return}d("poll")}function d(a){if("readystatechange"!=a.type||"complete"==b.readyState)("load"==a.type?c:b)[p](e+a.type,d,!1),!m&&(m=!0)&&h.call(c,a.type||a)}var m=!1,top=!0,b=c.document,l=b.documentElement,f=b.addEventListener,g=f?"addEventListener":"attachEvent",p=f?"removeEventListener":"detachEvent",e=f?"":"on";if("complete"==b.readyState)h.call(c,"lazy");else{if(!f&&l.doScroll){try{top=!c.frameElement}catch(a){}top&&k()}b[g](e+"DOMContentLoaded",d,!1);b[g](e+"readystatechange",d,!1);c[g](e+"load",d,!1)}};
	...
	GradientOverlay['Process'] = function (querySelectorResult, colors) {
		return GradientOverlay()['Process'](querySelectorResult, colors);
	};
	...
	GradientOverlay.prototype = {
		...
		'Process': function (querySelectorResult, colors) {
			...
			if (!node && querySelectorResult.length) {
				for (var i = 0, l = querySelectorResult.length; i < l; i += 1) 
					this._processInstance(querySelectorResult[i], gradient);
			} else if (node) {
				this._processInstance(querySelectorResult, gradient);
			} else {
				console.error(GradientOverlay.UNKNOWN_QUERY_SELECTOR_ERROR_MSG);
			}
		},
		_processInstance: function (elm, gradient) {
			if (elm[GradientOverlay['GRADIENT_OVERLAY_ID']]) return;
			var id = 'id' + (GradientOverlay.GRADIENT_OVERLAY_COUNTER += 1);
			GradientOverlay._notRenderedInstancesCount += 1;
			this._instances[id] = new GradientOverlay['Element'](id, elm, gradient);
		}
	};
	
	GradientOverlay['Element'] = function (id, elm, gradient) {
		this._id = id;
		this._elm = elm;
